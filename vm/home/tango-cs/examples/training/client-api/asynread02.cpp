 ...
 *  using the asynch call / callback - pull model
 * 
 * C. Scafuri
...
public:
    ...
    //
    // other virtual methods that can overloaded:
    //
...
int main(int argc, char **argv)
{
    ...
    //now recover the asynch reply by firing the callback
    try{
        dev->get_asynch_replies(40);  //wait 40 ms in this example
    }
    catch(AsynReplyNotArrived &e)
    {
        // if wait_time is too short AsynReplyNotArrived exception is rised
        Tango::Except::print_exception(e);
    ...
    // another way to recover the async data: use a polling loop
    try{
        dev->read_attribute_asynch(attname, rd_short_cb); //read the attribute asyncrhonously - callback variant
    }
    catch(DevFailed &e)
    ...
    for (unsigned int i=0; i< 1000; i++){
        dev->get_asynch_replies();  //poll without waiting, no excption if data is not ready
        usleep(1000);
    }
